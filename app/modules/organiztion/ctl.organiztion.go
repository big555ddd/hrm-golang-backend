...
func (ctl *Controller) Create(ctx *gin.Context) {
	body := organizationdto.CreateOrganization{}
...
func (ctl *Controller) Update(ctx *gin.Context) {
	ID := organizationdto.GetByIDOrganization{}
	...
	body := organizationdto.UpdateOrganization{}
	if err := ctx.Bind(&body); err != nil {
		logger.Errf(err.Error())
		response.BadRequest(ctx, err.Error(), nil)
		return
	}
	_, err := ctl.Service.Update(ctx, &body, ID.ID)
	if err != nil {
		logger.Errf(err.Error())
		response.InternalServerError(ctx, message.InternalServerError, nil)
		return
...
func (ctl *Controller) List(ctx *gin.Context) {
	req := organizationdto.ListOrganizationRequest{
		Page:    1,
		Size:    10,
		OrderBy: "asc",
		SortBy:  "created_at",
